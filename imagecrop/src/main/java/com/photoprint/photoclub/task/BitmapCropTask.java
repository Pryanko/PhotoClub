package com.photoprint.photoclub.task;
...
import com.photoprint.photoclub.callback.BitmapCropCallback;
import com.photoprint.photoclub.model.CropParameters;
import com.photoprint.photoclub.model.ExifInfo;
import com.photoprint.photoclub.model.ImageState;
import com.photoprint.photoclub.util.ImageHeaderParser;
import com.photoprint.utils.FileUtils;

import java.io.File;
import java.io.IOException;

/**
 * Класс для выполнения обрезания изображения
 * Обрезает часть изображения, которая заполняет границы кадрирования.
 *
 * Первое изображение уменьшается, если был задан максимальный размер
 * и если результирующее изображение больше максимального размера.
 *
 * Затем изображение поворачивается соответствующим образом.
 * Наконец, новый растровый объект создается и сохраняется в файл.
 *
